...
if rank == 0:
    ...
    subsample_array[:, 0] = subsamples["pid"]
    ...
    subsample_array = subsample_array[np.argsort(subsample_array[:, 0])]
    ...
    for i in range(size):
        ...
        lower_pid_indices[i] = np.sum((lower_pids[i] > subsample_array[:, 0]))
        N_subsamples[i] = np.sum((lower_pids[i] <= subsample_array[:, 0]) &
                                 (upper_pids[i] > subsample_array[:, 0]))

    subsample_array_list = np.split(subsample_array, lower_pid_indices[1:])
...
comm.Barrier()
lower_bound = comm.scatter(lower_bounds, root=0)
...
N_subsample = comm.scatter(N_subsamples, root=0)

subsample_array = np.empty((N_subsample, 7))
comm.Scatter(subsample_array_list, subsample_array, root=0)

del subsample_array_list
...
for i in range(lower_bound, upper_bound):
    ...
    with open("{}/ic_{}".format(ic_dir, i), "rb") as file:
        ...
        for j in range(last_sort_pid_index, lower_pid_index + N_subsample):
            if int(subsample_array[j, 0] - tot_particles) < N_particles:
                particle_data = unpack_from("3h6f", bdata,
                                            offset=int((subsample_array[j, 0] -
                                                        tot_particles)*32))
...
smooth_vel_array = np.empty((halos.shape[0], 4 * (len(smoothing_scales)-1)))
