...
//  double logMmax = 15.;
  ...
  cout<<"nBinsep: "<<nBinsep<<endl;
  cout<<"nBinM: "<<nBinM<<endl;
      ...
      for (int il=i_1;il<=i_2;il++){
        ...
        for (int jl=j_1;jl<=j_2;jl++){
          ...
          for (int kl=k_1;kl<=k_2;kl++){
            ...
            cout<<"im: "<<im<<"\tjm: "<<jm<<"\tkm: "<<km<<"\tj: "<<j<<endl;
              ...
              if (i<j && (halo.pid[j]==-1 || all_halos)){
                ...
                if (binMj == binM) {
                  ...
                  if (pair_sep>sepmin && pair_sep<sepmax){
                    ...
                    cout<<"xoff: "<<xoff<<"\tyoff: "<<yoff<<"\tzoff: "<<zoff<<"\tpair_sep: "<<pair_sep<<"\tbinsep: "<<binsep<<endl;
...
#pragma omp atomic
  ...
  cout<<"Fill Type: "<<fill_type<<"\t"<<"Index: "<<fill_type_index<<endl;
  ...
  for (int i=0; i<nBinM; i++) {
    ...
    for (int j=0; j<nBinsep; j++)
      pfw<<pow(10., j*logDsep + logSepmin)<<"\t"<<setprecision(6)<<fixed<<vp(j, i)<<"\t"<<0<<"\t"<<Nvp(j, i)<<endl;
